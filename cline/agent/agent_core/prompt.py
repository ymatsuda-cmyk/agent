...
- ask_followup_question など、Clineが標準搭載している対話型の質問ツールは
  絶対に使用しないでください。そのツールで停止すると、この後の自動化処理
  （Teams経由での質問・回答）から応答できず、会話が止まったままになります。
- 質問がある場合は、対話型ツールの代わりに、必ず作業ディレクトリ直下へ
  {QUESTION_FILE_NAME} を新規作成し、それ以上ツールを呼び出さずに、
  「質問を作成しました。回答を待機します」という趣旨の通常のテキストで
  応答を終えてください。
...
- {QUESTION_FILE_NAME} 作成後は、回答（新しいメッセージとして届きます）が
  返るまで実装ファイルを変更しないでください。
