...
**Clineの標準対話質問（`ask_followup_question`）は名指しで禁止している。**
実環境の検証で、Clineが `.agent-question.json` を作らず、標準搭載の
対話型質問ツールを呼んで停止してしまい、Teams経由の質問応答フロー
（②③）に乗らないまま会話が止まる事例があった。プロンプト
（`_question_contract()`）に「そのツールは絶対に使用しないでください。
質問後は通常のテキストで応答を終えてください」と明記して対応している。

**未回答の質問が残ったまま再実行されると、実装プロンプトの再送で
誤爆することがある。** Issueの処理が途中で止まり、`.agent-question.json`
が未回答のまま残った状態で `implement_agent.py` が再度起動されると
（デーモン再起動・`retry` 等）、以前は無条件で初回の実装プロンプトを
再送していた。しかしClineのウィンドウ側では前回の質問がまだ未回答で
開いている可能性が高く、そこへ「Issueを実装してください」を貼り付けると、
**その質問への回答として誤って解釈される**危険があった。

現在は `mode="new"` で起動する際、`.agent-question.json` が存在し、
かつその内容が今回のIssue宛ての有効な質問と判定できる場合は、
初回プロンプトを再送せず、質問処理（Teamsへの質問出力・回答待ち）を
そのまま継続する。無効な質問ファイル（別Issue宛て等）が残っている
場合は、これまで通りクリアして通常どおり再送する（無効なファイルの
せいで永久に再送できなくなることを防ぐため）。
